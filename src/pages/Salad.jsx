    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          ...
          const menuSubcollectionRef = collection(db, "menu", uid, "Salad");
          ...
          setSalad(fetchedMenus);
          setIsEmpty(fetchedMenus.length === 0); // Check if there are no menus in the subcollection

          console.log("Fetched Salad: ", fetchedMenus);
        } catch (error) {
          console.error("Error fetching Salad: ", error);
